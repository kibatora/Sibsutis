...
struct bstree* bstree_delete(struct bstree* tree, int key) {
  ...
  if (key < tree->key) {
    // Если ключ меньше ключа текущего узла, рекурсивно вызываем функцию для левого поддерева
    tree->left = bstree_delete(tree->left, key);
  } else if (key > tree->key) {
    // Если ключ больше ключа текущего узла, рекурсивно вызываем функцию для правого поддерева
    ...
    if (tree->left == NULL && tree->right == NULL) {
      ...
      // Если у узла есть только правый ребенок, заменяем узел на его правого ребенка
      struct bstree *right_child = tree->right;
      free(tree);
      return right_child;
    } else if (tree->right == NULL) {
      // Если у узла есть только левый ребенок, заменяем узел на его левого ребенка
